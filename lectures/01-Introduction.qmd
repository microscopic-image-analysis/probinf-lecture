...
\begin{aligned}
	...
	&= \frac{1}{S^2} \sum_{s,s'} \mathbb{E}_{p_S}\bigl[f(x^{(s)})\,f(x^{(s')}) - \mu(f(x^{(s)}) + f(x^{(s')})) + \mu^2)\bigr] \\
	&= \frac{1}{S^2} \sum_{s,s'} \mathbb{E}_{p_S}\bigl[f(x^{(s)})\,f(x^{(s')})\bigr] - \mu^2 \\
	&= \frac{1}{S^2} \sum_{s} \mathbb{E}_{p_S}\bigl[\bigl(f(x^{(s)})\bigr)^2\bigr] + \frac{1}{S^2} \sum_{s\not= s'} \mathbb{E}_{p_S}\bigl[f(x^{(s)})\,f(x^{(s')})\bigr] - \mu^2 \\
	&= \frac{1}{S^2} \sum_{s} \mathbb{E}_{p}\bigl[\bigl(f(x^{(s)})\bigr)^2\bigr] + \frac{S(S-1)}{S^2} \mu^2 - \mu^2 \\
	&= \frac{1}{S} \mathbb{E}_{p}\bigl[f^2\bigr] - \frac{1}{S} \mu^2 \\
	&= \frac{1}{S} \mathbb{E}_{p}\bigl[(f - \mu)^2\bigr] \\
    &= \frac{1}{S} \text{var}_{p}[f] 
\end{aligned}
$$ {#eq-MCvariance}
where we've made use of the linearity of expectations in the third equality, and of the independence of the samples (eq. @eq-MC-sample-independece) in the seventh.
